...
- `packages/agent`: LangChain/LangGraph agent runtime, models, tools, and graph config.
...
- Require code-owner review before merging changes under `packages/agent`.
- Restrict direct pushes to protected branches.

If teams must have strict repository-level read/write isolation, use one of these models instead:

- Keep this root repository as an orchestrator and mount `apps/web` and `packages/agent` as Git submodules.
...
- Add new Agent implementations under `packages/agent/src/agents` or create another package under `packages/*` if ownership/runtime differs.
- Keep package imports directional: Web may depend on Agent, but Agent must not import from Web.
